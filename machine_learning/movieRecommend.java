...
public class movieRecommend {
	...
	public void testNB(String trainingSet, String testSet, String userId) throws Exception{
		...
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Reading training set completed...");
		
		nb.buildClassifier(newTrain);
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Training Naive Bayes completed...");
		dt.buildClassifier(newTrain);
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Training Decision Tree completed...");
		rf.buildClassifier(newTrain);
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Training RandomForest completed...");
		...
		//Instances newTest = Filter.useFilter(test, remove);
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Reading test set completed...");
		...
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Storing classification result completed...");
		...
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Cross-validating Naive Bayes...");
		eval.crossValidateModel(nb, newTrain, 10, new Random(1));
		writer.write(eval.toSummaryString("\n Naive Bayes Result\n============================\n", true));
		
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Cross-validating Decision Tree...");
		eval.crossValidateModel(dt, newTrain, 10, new Random(1));
		writer.write(eval.toSummaryString("\n Decision Tree Result\n============================\n", true));
		
		System.out.println("[Final Classification]["+new Timestamp(date.getTime())+"] Cross-validating Random Forest...");
